...
namespace WinFormsApp1.Properties
{
    public class DataEncryptionStandard
    {
        ...
        */
        ...
        {
            // message - wiadomość do zakodowania/zdekodowania
            // key - klucz do (de)szyfrowania
            // cryptor - tryb pracy: true -> szyfrowanie false -> rozszyfrowywanie

            //zmienna przechowująca znaki w postaci binarnego kodu ascii
            byte[] startArr = new byte[8];

            //do indexowania znaków w wiadomości
            int index = 0;

            //fragment wiadomości w postaci 64bitowego bloku danych
            UInt64 microMessage = 0;

            string output = "";

            //uzupełnienie wiadomości spacjami do wielokrotności ósemki
            int rest = 8 - message.Length % 8;
            for (int i = 0; rest != 8 && i < rest; i++)
                message += ' ';
            Console.WriteLine(message.Length);

            // wyciąganie 64bitowych fragmentów wiadomości
            for (int a = 0; a < (message.Length / 8); a++)
            {
                ...
                //przepuszczenie fragmentu wiadomości przez bloki (de)szfrujące
